...
// ViewerDatabaseReader.swift - Read-only database access for Viewer window
...
final class ViewerDatabaseReader {
    ...
    static func loadTags() async -> [TagData] {
        guard let dbPath = databasePath,
              FileManager.default.fileExists(atPath: dbPath) else {
            return []
        }
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            let tag = TagData(
                ...
                isRetired: sqlite3_column_int(stmt, 3) != 0
            )
            tags.append(tag)
        }
        return tags
    ...
    // MARK: - Private Query Methods (same as TodayTotalCalculator)
    ...
    private static func loadRawActivityEvents(
        ...
        guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else {
            throw NSError(domain: "ViewerDatabaseReader", code: 2)
        }
        sqlite3_bind_int64(stmt, 1, start)
        sqlite3_bind_int64(stmt, 2, end)
    ...
    private static func loadUserEditEvents(
        ...
        guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else {
            throw NSError(domain: "ViewerDatabaseReader", code: 3)
        }
